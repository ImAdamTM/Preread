...
There is a standalone CLI tool at `tools/cache-debugger/` for inspecting what happens when Preread caches a URL. It replicates the exact standard-mode pipeline from `PageCacheService` and saves intermediate HTML files.

When the user says "debug this URL" (or similar), run:

```bash
cd /Users/adamhartwig/gitrepos/Preread/tools/cache-debugger && swift run CacheDebugger "<URL>"
```

For full-page mode, add `--full`:

```bash
cd /Users/adamhartwig/gitrepos/Preread/tools/cache-debugger && swift run CacheDebugger "<URL>" --full
```

This produces 4 files in `tools/cache-debugger/output/`:
...
3. **CRITICAL: Apply the same change to `tools/cache-debugger/Sources/main.swift`**. The debug tool replicates the production pipeline and must stay in sync — hero image filtering, cleaning rules, image stripping, and all other pipeline logic must match `PageCacheService.swift` exactly.
4. Fixture files live in `PrereadTests/Fixtures/` — raw HTML saved from the cache-debugger tool's `1_raw.html` output

### Adding a new fixture

1. Run the cache-debugger tool against the URL: `cd tools/cache-debugger && swift run CacheDebugger "<URL>"`
2. Copy `tools/cache-debugger/output/1_raw.html` to `PrereadTests/Fixtures/<name>.html`
...
The feed directory lives at `tools/update-feed-directory/`. Feeds are organised as per-category JSON files in `tools/update-feed-directory/categories/*.json`.
